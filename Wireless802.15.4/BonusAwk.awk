BEGIN {
	...
	for (i=0; i<max_node; i++) {
		nReceivedBytesPerNode[i] = 0;	
		rStartTimePerNode[i] = 10000;
		rEndTimePerNode[i] = 0;
...
#		printf("%d %15.5f\n", node, energy_consumption[node]);
	...
	if ( strAgt == "AGT"   &&   strType == "tcp" ) {
		...
		if(rTime>rEndTimePerNode[node]) rEndTimePerNode[node]=rTime;
		if(rTime<rStartTimePerNode[node]) rStartTimePerNode[node]=rTime;
...
#		if ( strEvent == "r" ) {
		if ( strEvent == "r" && idPacket >= idLowestPacket) {
			nReceivedBytesPerNode[node] += nBytes;
...
END {
	...
	for (i=0; i<= 100; i++) {
 		...
 		rTimePerNode =  rEndTimePerNode[i]-rStartTimePerNode[i];
		#rThroughputPerNode = nReceivedBytesPerNode[i]*8 / rTimePerNode;
		rThroughputPerNode = nReceivedBytesPerNode[i]*8 / rTime;
		#printf("Node: %d Bytes %d Time: %d Throughput: % 15.5f\n",i,nReceivedBytesPerNode[i],rTimePerNode,rThroughputPerNode);
		#printf("Node: %d Bytes %d Time: %d Throughput: % 15.5f\n",i,nReceivedBytesPerNode[i],rTimePerNode,rThroughputPerNode);
		printf("%d  %15.5f\n",i,rThroughputPerNode) > "Nodes_vs_Throughput.dat";
